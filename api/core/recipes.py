...
# ──────────────────────────────────────────────────────────────
# 6) AI 식단 추천 (meal recommendation)
# ──────────────────────────────────────────────────────────────
_MEAL_AI_CACHE: dict[str, list[dict]] = {}

_PREF_DESC: dict[str, str] = {
    "균형":   "균형 잡힌 한식 (단백질·채소·탄수화물 골고루)",
    "채소":   "채식 위주 (육류 없이 채소·두부·계란 중심)",
    "저단백": "저단백 식단 (단백질 최소, 채소·탄수화물 위주, 육류 제외)",
    "고단백": "고단백 식단 (닭가슴살·계란·두부 등 단백질 풍부, 탄수화물 적게)",
}


def recommend_meals_ai(pref: str, budget: int, household: int) -> list[dict]:
    """식단 선호 → 추천 음식 2개 + 재료 리스트. OpenAI 우선, 룰 기반 폴백."""
    cache_key = f"{pref}:{budget}:{household}"
    if cache_key in _MEAL_AI_CACHE:
        return _MEAL_AI_CACHE[cache_key]

    desc = _PREF_DESC.get(pref, _PREF_DESC["균형"])
    raw = _call_openai(
        system=(
            "당신은 한국 영양사입니다. "
            "식단 조건에 맞는 한국 음식 2가지를 추천하고 재료를 알려주세요. "
            '형식: {"meals": [{"dish": "음식명", "ingredients": ["재료1", "재료2", ...]}, ...]} '
            "재료는 한국 마트에서 살 수 있는 실제 식재료 이름으로 간결하게. "
            "소금·물·식용유 같은 기본 양념 제외. 각 음식당 재료 4~7개."
        ),
    ...
    if raw:
        try:
            ...
            for m in obj.get("meals", [])[:2]:
                dish = m.get("dish", "")
                ings = [_normalize_ingredient(i) for i in m.get("ingredients", []) if i]
                if dish and ings:
    ...
    if not meals:
        _FALLBACK: dict[str, list[dict]] = {
            "균형":   [{"dish": "된장찌개", "ingredients": ["두부", "애호박", "양파", "대파", "된장", "감자"]},
                       {"dish": "제육볶음",  "ingredients": ["돼지고기앞다리", "양파", "대파", "고추장", "마늘"]}],
            "채소":   [{"dish": "시금치나물", "ingredients": ["시금치", "마늘", "간장", "참기름"]},
                       {"dish": "된장찌개",   "ingredients": ["두부", "애호박", "양파", "대파", "된장"]}],
            "저단백": [{"dish": "감자조림",  "ingredients": ["감자", "간장", "마늘", "양파"]},
                       {"dish": "콩나물국밥", "ingredients": ["콩나물", "계란", "대파", "쌀", "고춧가루"]}],
            "고단백": [{"dish": "닭가슴살 샐러드", "ingredients": ["닭가슴살", "시금치", "양파", "사과"]},
                       {"dish": "계란찜",            "ingredients": ["계란", "대파", "간장", "두부"]}],
        }
        meals = _FALLBACK.get(pref, _FALLBACK["균형"])

    _MEAL_AI_CACHE[cache_key] = meals
    return meals


def lookup_ingredient_info(name: str, items_df: "pd.DataFrame") -> dict:
    """재료명 → {name, price, unit, emoji}. DB → EXTRA_ITEMS → AI 추론 순."""
    from core.items import EMOJI_MAP
    emoji = EMOJI_MAP.get(name, "🛒")

    if not items_df.empty:
        row = items_df[items_df["name"] == name]
        if row.empty:
            row = items_df[items_df["name"].str.contains(name, na=False, regex=False)]
        if not row.empty:
            r = row.iloc[0]
            price = int(r.get("avg_price", 0))
            if price <= 0:
                price = int((r.get("market_price", 0) + r.get("supermarket_price", 0)) / 2)
            return {
                "name": name,
                "price": price or 2000,
                "unit": str(r.get("unit", "")),
                "emoji": EMOJI_MAP.get(name, str(r.get("emoji", "🛒"))),
            }

    if name in EXTRA_ITEMS:
        ei = EXTRA_ITEMS[name]
        return {"name": name, "price": ei[2], "unit": ei[1], "emoji": ei[5]}

    price = _infer_item_price_ai(name)
    return {"name": name, "price": price, "unit": "", "emoji": emoji}
