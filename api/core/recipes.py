...
def _infer_cooking_unit(item_key: str, db_price: int, db_unit: str, household: int = 2) -> dict:
    ...
    result = _call_openai(
        system=(
            ...
            "예: 양파(1kg/2200원) → unit=1개(200g), price=440 "
            "예: 생강(1kg/8000원) → unit=50g, price=400 "
            "예: 계란(30개/7200원) → unit=6개, price=1440 "
            "예: 대파(1단/2500원) → unit=1/3단, price=830 "
            "규칙: 양념류(간장·된장·고추장·참기름 등)는 1회 사용량 기준, "
            "채소는 실제 1~2인 요리에 쓰는 양, 육류는 1인 150g 기준. "
            "단위는 한국어로 간결하게. 가격은 정수(원)."
            "응답 형식(JSON만): unit 필드와 price 필드"
        ),
