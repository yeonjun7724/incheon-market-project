"""routers/admin.py — 가격 동기화 수동 트리거 + GeoJSON + ZIP"""
import io
import os
import zipfile
import json
...
def sync_prices(target_date: str | None = Query(None, description="YYYY-MM-DD, 기본=오늘")):
    """
    PostGIS daily_prices 테이블을 KAT+KAMIS 최신 데이터로 동기 갱신.
    완료될 때까지 대기 후 결과 반환.
    """
    result = run_price_sync(target_date)
    if result.get("ok"):
        return {"ok": True, "rows": result.get("rows", 0),
                "message": f"갱신 완료 — {result.get('rows', 0)}행 저장됨"}
    else:
        return {"ok": False, "rows": 0,
                "message": f"갱신 실패: {result.get('error', '알 수 없는 오류')}"}


@router.get("/prices/geojson")
def prices_geojson(category: str | None = Query(None)):
    ...
    stores_df = stores_df.rename(columns={
        "점포명": "name", "위도": "lat", "경도": "lng",
        "유형": "type", "위치": "address"
    })

    features = []
    for _, row in df.iterrows():
        matched = stores_df[stores_df["name"].str.contains(
            str(row.get("gds_lclsf_nm", "")), na=False)]
