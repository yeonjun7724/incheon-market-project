...
export default function MapCanvas({ priceLayerOn }: { priceLayerOn: boolean }) {
  ...
  return (
    <>
      ...
      >
        ...
        <Source id="stores" type="geojson" data={geojson}
          ...
          <Layer id="cluster-ring" type="circle" filter={!routeChoice ? ["has", "point_count"] : ["==", "id", ""]}
            paint={{
              "circle-color": "rgba(193,18,31,0.22)",
              "circle-radius": ["step", ["get", "point_count"], 30, 10, 39, 30, 50],
              "circle-stroke-width": 1,
              "circle-stroke-color": "rgba(193,18,31,0.45)",
            }}
          />
      ...
      {/* 경로 요약 뱃지 */}
      {mapboxRoute && plan && (() => {
        const isD   = travelMode === 'driving';
        const walkM = Math.round(mapboxRoute.duration_s / 60);
        const shopM = plan.n_stops * 10;
        const distKm = (mapboxRoute.distance_m / 1000).toFixed(1);
        const modeIcon = isD ? '🚗' : '🚶';
        const modeLabel = isD ? '운전' : '도보';
        return (
          <div style={{
            position: 'fixed',
            bottom: 88,
            ...
            borderRadius: 16,
            overflow: 'hidden',
            boxShadow: '0 4px 20px rgba(26,34,51,0.18)',
            ...
            <div style={{
              ...
              padding: '8px 12px',
              background: isD ? '#e85d04' : '#0077b6',
              gap: 2,
            }}>
              <span style={{ fontSize: 16 }}>{modeIcon}</span>
              <span style={{ fontSize: 10, fontWeight: 700, color: '#fff', letterSpacing: 0.5 }}>{modeLabel}</span>
            </div>
            {/* 지표들 */}
            {[
              { label: '거리', val: `${distKm}km` },
              { label: modeLabel, val: `${walkM}분` },
              { label: '구입', val: `${shopM}분` },
              { label: '총소요', val: `${walkM + shopM}분` },
            ].map(({ label, val }, i, arr) => (
              <div key={label} style={{
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                padding: '8px 14px',
                borderRight: i < arr.length - 1 ? '1px solid rgba(26,34,51,0.07)' : 'none',
                minWidth: 52,
              }}>
                <span style={{ fontSize: 15, fontWeight: 900, color: '#1a2233', fontVariantNumeric: 'tabular-nums' }}>
                  {val}
                </span>
                <span style={{ fontSize: 10, color: '#8a96b0', marginTop: 2 }}>{label}</span>
              </div>
            ))}
          </div>
        );
